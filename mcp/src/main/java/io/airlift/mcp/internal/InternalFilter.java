        ...
        extends HttpFilter
{
    ...
    @Inject
    public InternalFilter(McpMetadata metadata, Optional<McpIdentityMapper> identityMapper, InternalMcpServer mcpServer, ObjectMapper objectMapper, TaskEmulationDecorator taskEmulationDecorator, Optional<SessionController> sessionController)
    {
    ...
    private void handleRpcResponse(HttpServletRequest request, HttpServletResponse response, JsonRpcResponse<?> rpcResponse)
    {
        ...
        taskEmulationDecorator.handleMcpResponse(rpcResponse, () -> responseError(response, SC_BAD_REQUEST, invalidRequest("The server does not accept responses")));
    }
